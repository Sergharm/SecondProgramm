...
static void init_table(ConstBuffer key, uint8_t table[16][16]) {
    ...
    for (int i = 0; i < 256; ++i) {
        if (!used[i]) {
            flat_table[count++] = static_cast<uint8_t>(i);
        }
    }
    for (int r = 0; r < 16; ++r) {
        for (int c = 0; c < 16; ++c) {
...
size_t get_output_size(size_t input_size, int operation_type) {
    ...
    if (operation_type == 1) { 
        size_t pad_len = BLOCK_SIZE - (input_size % BLOCK_SIZE);
        return input_size + pad_len;
    }
    return input_size;
}
...
int encrypt(ConstBuffer key, ConstBuffer input, MutBuffer* output) {
    ...
    for (size_t i = 0; i < padded_size; i += BLOCK_SIZE) {
        uint8_t block[2];
        block[0] = (i < input.size) ? input.data[i] : static_cast<uint8_t>(pad_len);
        block[1] = (i + 1 < input.size) ? input.data[i + 1] : static_cast<uint8_t>(pad_len);

        int r1, c1, r2, c2;
        find_position(table, block[0], r1, c1);
        find_position(table, block[1], r2, c2);
...
int decrypt(ConstBuffer key, ConstBuffer input, MutBuffer* output) {
    ...
    output->size = input.size - pad_val;
    secure_clear_memory(table, sizeof(table));
    return 0;
}
